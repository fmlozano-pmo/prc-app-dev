...
const Charts = (() => {
  ...
  function awardingLeadTime(id,wps){
    ...
    make(id,{type:'bar',data:{labels:data.map(w=>w.wp_no),datasets:[{label:'Lead time',data:vals,borderRadius:3,backgroundColor:vals.map(v=>v>0?'rgba(226,75,74,0.75)':'rgba(29,158,117,0.75)')}]},
      options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},
  ...
  function budgetVsContract(id,wps){
    ...
    make(id,{type:'bar',data:{labels:data.map(w=>w.wp_no),datasets:[
      {label:'Budget',data:data.map(w=>+(w.approved_budget_php/1e6).toFixed(2)),backgroundColor:'rgba(55,138,221,0.55)',borderRadius:3},
      {label:'Contract',data:data.map(w=>+(w.contract_amount_php/1e6).toFixed(2)),backgroundColor:'rgba(29,158,117,0.75)',borderRadius:3},
  ...
  function awardDonut(id,stats){
    make(id,{type:'doughnut',data:{labels:['Awarded','Partially Awarded','Not Yet Awarded'],
      datasets:[{data:[stats.awarded.length,stats.partial.length,stats.notYet.length],backgroundColor:['#1D9E75','#EF9F27','#B4B2A9'],borderWidth:0}]},
  ...
  function consolidatedBudget(id,projects,allStats){
    make(id,{type:'bar',data:{labels:projects.map(p=>p.id),datasets:[
      {label:'Budget',data:allStats.map(s=>+(s.totalBudget/1e6).toFixed(1)),backgroundColor:'rgba(55,138,221,0.55)',borderRadius:4},
      {label:'Contract',data:allStats.map(s=>+(s.totalContract/1e6).toFixed(1)),backgroundColor:'rgba(29,158,117,0.75)',borderRadius:4},
    ]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},
