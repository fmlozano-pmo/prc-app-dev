...
17. **`procurement_status` vs `award_status`**: Dashboard KPIs (`buildMetrics`, `computeStats`) read only `award_status` (`Not Yet Awarded | Partially Awarded | Awarded`). `procurement_status` (`Not Started | Sourcing | … | Awarded`) is a separate field. `onProcStatusChange()` in `wp-form.html` auto-syncs: setting proc_status → "Awarded" sets award_status → "Awarded" (and reverts when changed back). Edit pre-fill also corrects stale award_status on load.
